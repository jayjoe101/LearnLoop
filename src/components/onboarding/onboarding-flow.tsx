...
export function OnboardingFlow() {
  ...
  const canProceed = selectedTopics.length >= MIN_ONBOARDING_TOPICS;
  const selectionProgress = Math.min(
    100,
    (selectedTopics.length / MIN_ONBOARDING_TOPICS) * 100
  );
  ...
  function toggleTopic(topic: string) {
    setSelectedTopics((prev) =>
      prev.includes(topic)
        ...
        : prev.length < MAX_ONBOARDING_TOPICS
          ? [...prev, topic]
          : prev
    );
  }
  ...
  function addCustomTopic() {
    ...
    if (selectedTopics.length >= MAX_ONBOARDING_TOPICS) return;
  ...
  function goNext() {
    ...
    else if (step === "interests" && canProceed) {
      setStep("building");
      startBuilding();
    }
  }
      ...
      <div className="onboarding-card w-full max-w-lg">
        <div className="mb-8 flex gap-1.5">
          {[0, 1].map((i) => {
            ...
            return (
              <div
                key={i}
                className={`onboarding-step-indicator h-0.5 flex-1 rounded-full transition-all duration-500 ${
                  filled ? "onboarding-step-indicator--filled" : ""
                }`}
              />
            );
          })}
        ...
        {step === "welcome" && (
          <div key="welcome" className="onboarding-step">
            ...
            <p className="mx-auto mt-3 max-w-sm text-center text-sm leading-relaxed text-[var(--color-coffee-text-soft)]">
        ...
        {step === "interests" && (
          <div key="interests" className="onboarding-step">
            ...
            <p className="mt-2 text-sm text-[var(--color-coffee-text-soft)]">
              Pick at least {MIN_ONBOARDING_TOPICS} interest
              {MIN_ONBOARDING_TOPICS === 1 ? "" : "s"}. Each post will explore a
              specific subject inside one of them.
            </p>
            ...
            <div className="mt-4 flex gap-2">
              ...
              <button
                ...
                disabled={
                  !customTopic.trim() ||
                  selectedTopics.length >= MAX_ONBOARDING_TOPICS
                }
            ...
            <div className="onboarding-progress-track mx-auto mt-6 h-1 max-w-xs overflow-hidden rounded-full">
              <div
            ...
            <p className="mt-3 text-center text-xs text-[var(--color-coffee-taupe)]">
              {selectedTopics.length} selected
              {!canProceed &&
                ` · pick ${MIN_ONBOARDING_TOPICS - selectedTopics.length} more to continue`}
            </p>
            ...
            <button
              ...
              className="onboarding-btn-primary mt-6 w-full disabled:opacity-40"
            >
              Build my feed
            </button>
          </div>
